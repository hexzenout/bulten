...
(function () {
  ...
  if (!window.__omegaV880PendingCloseGuardBound) {
    window.__omegaV880PendingCloseGuardBound = true;
    const pendingCloseGuard = function(event) {
      ...
      const clickedPendingBlank = !!target.closest(".v758-pending-overlay") && !pendingModal && !confirmLayer;
      const clickedHostBlank = target === host || (!!target.closest("#omega-rolling-feature-host") && !pendingModal && !confirmLayer);

      if (!closeBtn && !clickedPendingBlank && !clickedHostBlank) return;
  ...
  function renderModule() {
    ...
    const betTotalPnl = betSum.pnl;
    const cryptoTotalPnl = cryptoSum.pnl;
