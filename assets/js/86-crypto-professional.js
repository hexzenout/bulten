...
(function () {
  ...
  function v903BetSummaryHtml(row, kind) {
    ...
    if (isHistory) {
      const titlePhotoBtn = `<button type="button" class="v919-summary-camera" data-v919-summary-photo="${row.day}:${row.slot}" title="Geçmiş kupon fotoğrafı" aria-label="Geçmiş kupon fotoğrafı"><i class="fa-solid fa-camera"></i></button>`;
      return `<div class="v903-bet-summary" data-v903-accordion-toggle aria-expanded="false" role="button" tabindex="0">
        <strong class="v919-summary-title-row"><span class="v919-summary-title-text"><span class="v911-summary-date">${stamp} - </span><span class="v911-summary-bet-ref" style="color:#fbbf24 !important;">Gün ${row.day} · Bahis ${row.slot + 1}</span></span>${titlePhotoBtn}</strong>
  ...
  if (!window.__omegaV768ExcelFeatureDelegationBound) {
    ...
    document.addEventListener("click", function(event) {
      ...
      const summaryPhotoBtn = event.target.closest && event.target.closest("[data-v919-summary-photo]");
      if (summaryPhotoBtn) {
        event.preventDefault();
        event.stopPropagation();
        if (typeof event.stopImmediatePropagation === "function") event.stopImmediatePropagation();
        const [dayRaw, slotRaw] = String(summaryPhotoBtn.dataset.v919SummaryPhoto || "0:0").split(":");
