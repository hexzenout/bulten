...
(function () {
  ...
  function v774NormalizePendingEntry(entry) {
    ...
    const comboResults = Array.isArray(entry.comboResults)
      ? entry.comboResults.map(v => v === "loss" ? "loss" : v === "win" ? "win" : "")
      : [];
  ...
  function v774PendingFromDom(day, slot) {
    ...
    const old = v774GetPendingSlot(day, slot);
    const comboResults = Array.isArray(old?.comboResults) ? old.comboResults : [];
            ...
            <div class="kapsul v32 ${isCryptoV491 ? "" : "v765-bet-kapsul"}" data-v765-kapsul="${day}:${slot}">
              ...
              ` : `
                <div class="v765-bet-entry">
                  ...
                  <div class="v847-bet-leg-result-panel" data-v847-leg-panel="${day}:${slot}">${v847RenderBetLegResultPanel(day, slot)}</div>
                </div>
              `}
  ...
  window.omega_ResolveExcelOp = function(day, slot, result) {
    ...
    const comboResults = Array.isArray(pendingBeforeResolve?.comboResults) ? pendingBeforeResolve.comboResults : [];
