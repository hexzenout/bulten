...
(function () {
  ...
  function renderSoundPanel(force = false) {
    ...
    v499fSettings({ sound: soundType, volume: 1, enabled: true });
    ...
    mount.innerHTML = `
      <div class="v499f-sound-card">
        ...
        <div class="v499f-actions">
          <button id="v32-sound-test" type="button">OYNAT</button>
          <button id="v32-sound-stop" type="button" class="danger">DURDUR</button>
        </div>

        <div class="v499f-field">
        ...
        <div class="v499f-custom ${isCustom ? "show" : ""}" id="v47-custom-sound-panel">
          <div class="v499f-library-title"><i class="fa-solid fa-music"></i><b>Özel Ses Kütüphanesi</b></div>

          <div class="v499f-select-row">
            <button type="button" id="v499f-play-custom" class="v499f-play" title="Seçili özel sesi oynat / durdur"><i class="fa-solid fa-play"></i><i class="fa-solid fa-pause"></i></button>
          ...
          <div class="v499f-player">
            <div class="v499f-player-time">
              <span id="v499f-current">0:00</span>
              <b>Alarm Aralığı: <strong id="v499f-range-label">0:00 - 0:00</strong></b>
              <span id="v499f-total">0:00</span>
            </div>

            <div class="v499f-track" id="v499f-track">
              <div class="v499f-wave"></div>
              <div class="v499f-fill" id="v499f-fill"></div>
              <button type="button" class="v499f-handle start" id="v499f-start-handle">[</button>
              <button type="button" class="v499f-handle end" id="v499f-end-handle">]</button>
              <input id="v499f-progress" type="range" min="0" max="1" step="1" value="0" title="Dinleme konumu">
              <input id="v499f-start" type="range" min="0" max="1" step="1" value="0" hidden>
              <input id="v499f-end" type="range" min="0" max="1" step="1" value="1" hidden>
            </div>

            <button type="button" id="v499f-apply">BU ARALIĞI ALARM YAP</button>
          </div>

          <div class="v499f-file-row">
            <button type="button" id="v32-file-pick">DOSYA SEÇ</button>
            <span id="v499f-file-note">Dosya seçilmedi</span>
    ...
    qs("#v32-sound-test").onclick = v499fPlaySelectedSound;

    qs("#v32-sound-type").onchange = e => {
      v499fSettings({ sound: e.target.value, volume: 1, enabled: true });
      renderSoundPanel(true);
    };

    qs("#v499f-play-custom").onclick = async e => {
      e.preventDefault();
      if (qs("#v499f-play-custom")?.classList.contains("playing")) {
        v499fStopPreview();
      } else {
        await v499fPlayCustomPreview();
      }
    };
    ...
    qs("#v32-file-input").onchange = async e => {
      ...
      try {
        ...
        v499fSettings({ sound: "custom", selectedCustomId: row?.id || "", customStart: 0, customEnd: 0, volume: 1, enabled: true });
    ...
    qs("#v47-custom-select").onchange = async e => {
      ...
      v499fSettings({ sound: "custom", selectedCustomId: id, volume: 1, enabled: true });
      await renderSoundLibrary();
      await v499fLoadMeta();
    };
    ...
    qs("#v499f-progress").addEventListener("input", e => {
      const sec = Number(e.target.value || 0);
      if (v499fPreviewAudio) v499fPreviewAudio.currentTime = sec;
      v499fUpdateUI(sec);
    });

    ["#v499f-start", "#v499f-end"].forEach(sel => {
      qs(sel)?.addEventListener("input", () => v499fUpdateUI());
    });

    qs("#v499f-apply").onclick = () => {
      const { start, end } = v499fGetSegment();
      v499fSettings({ sound: "custom", customStart: start, customEnd: end, volume: 1, enabled: true });
      const btn = qs("#v499f-apply");
      if (btn) {
        btn.textContent = `AKTİF: ${v499fFormat(start)} - ${v499fFormat(end)}`;
        setTimeout(() => btn.textContent = "BU ARALIĞI ALARM YAP", 1600);
      }
    };

    renderSoundLibrary().then(() => v499fLoadMeta().then(v499fBindTimeline));
  }
